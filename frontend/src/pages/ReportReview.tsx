...
export function ReportReview() {
  ...
  // Eligible for bulk approval: deterministic strategies only, in a reviewable status.
  // Narrative LLM fields are intentionally excluded — they must be reviewed individually.
  const autoApproveEligible = fields.filter(
    (f) => (f.status === 'draft' || f.status === 'edited') && f.strategy !== 'narrative_llm',
  )
  const narrativesUnapproved = fields.filter(
    (f) => f.strategy === 'narrative_llm' && f.status !== 'approved',
  ).length
  ...
  async function handleApproveAutoGenerated() {
    if (autoApproveEligible.length === 0) return
    setBulkApproving(true)
    try {
      const results = await Promise.allSettled(
        autoApproveEligible.map((f) =>
          updateField(id!, f.field_id, { status: 'approved' }),
        ),
      )
      for (const r of results) {
        if (r.status === 'fulfilled') upsertField(r.value)
      }
      ...
      <header className="flex-none border-b bg-background px-6 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {autoApproveEligible.length > 0 && (
              <div className="relative">
                <button
                  onClick={() => setBulkConfirmOpen((o) => !o)}
                  disabled={bulkApproving}
                  title="Approve all deterministic fields; LLM narratives still require manual review"
                  className="rounded border px-3 py-1.5 text-xs font-medium transition-colors hover:bg-muted disabled:opacity-50"
                >
                  {bulkApproving
                    ? 'Approving…'
                    : `Approve auto-generated (${autoApproveEligible.length})`}
                </button>
                {bulkConfirmOpen && !bulkApproving && (
                  <div className="absolute right-0 top-full z-50 mt-1 w-72 overflow-hidden rounded-md border bg-card p-3 shadow-lg">
                    <p className="text-xs font-medium text-foreground">
                      Approve {autoApproveEligible.length} auto-generated field
                      {autoApproveEligible.length === 1 ? '' : 's'}?
                    </p>
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      {narrativesUnapproved > 0
                        ? `${narrativesUnapproved} LLM narrative${narrativesUnapproved === 1 ? '' : 's'} will still require manual review before export.`
                        : 'All LLM narratives are already approved.'}
                    </p>
                    <div className="mt-3 flex gap-2">
                      <button
                        onClick={handleApproveAutoGenerated}
